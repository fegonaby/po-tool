...
SELECT q.ID AS QUEUE_ID,
       ...
       (SELECT FI_REFERENCE_NUMBER FROM (
           SELECT ROW_NUMBER() OVER (ORDER BY FI_FI_REF_SEQ) rn, FI_REFERENCE_NUMBER
           FROM {_FI_REF}
           WHERE PAYMENT_REFERENCE_NUMBER = t.REFERENCE_NUMBER
       ) WHERE rn = 1) AS SENDER_FI_REF,
       (SELECT FI_REFERENCE_NUMBER FROM (
           SELECT ROW_NUMBER() OVER (ORDER BY FI_FI_REF_SEQ) rn, FI_REFERENCE_NUMBER
           FROM {_FI_REF}
           WHERE PAYMENT_REFERENCE_NUMBER = t.REFERENCE_NUMBER
       ) WHERE rn = 2) AS RECIPIENT_FI_REF
FROM {_EMT} t
